class ChumZone {
    ...
    updateState(realTimeNow, timeScale) {
        ...
        if (gameElapsedMs < peakStartTime) {
            ...
            const progress = (gameElapsedMs - peakEndTime) / (totalTime - peakEndTime);
    ...
    getMultiplierAt(targetX, targetY, targetFishId, virtualTopY, virtualBottomY) {
        ...
        const distRatio = Math.max(0, Math.min(1.0, (this.y - virtualTopY) / (virtualBottomY - virtualTopY)));
        const currentRadY = this.baseRadYMin + ((this.baseRadYMax - this.baseRadYMin) * distRatio);

        const dx = targetX - this.x;
        const dy = targetY - this.y;

        const isInside = ((dx * dx) / (this.baseRadX * this.baseRadX)) + ((dy * dy) / (currentRadY * currentRadY)) <= 1;
...
class ChumManager {
    ...
    saveToStorage() {
        ...
        const zonesToSave = this.#zones.filter(z => !z.isExpired).map(z => ({
            id: z.id,
            x: z.x,
            y: z.y,
        ...
        localStorage.setItem(this.#locationMemoryKey, JSON.stringify(this.#memoryGrid));
    }
    ...
    deployBait(targetX, targetY, baitId, method, activeBoat = null) {
        ...
        const newZone = new ChumZone(zoneId, targetX, targetY, baitConfig, Date.now(), isHand);

        const overlappingIndex = this.#zones.findIndex(z => z.checkOverlap(newZone));
    ...
    isBoatMoving() {
        return this.#boats.some(b => b.state === 'deploying' || b.state === 'returning');
    }
    ...
    getMultiplier(floatX, floatY, fishId, virtualTopY, virtualBottomY) {
        ...
        for (const zone of this.#zones) {
            const zoneMult = zone.getMultiplierAt(floatX, floatY, fishId, virtualTopY, virtualBottomY);
    ...
    update(dt, checkWater, cellSize, env) {
        ...
        if (this.state === 'waiting' || this.state === 'drifting') {
            if (env && env.current) {
                ...
                if (this.state === 'drifting') {
                    this.angle = Math.atan2(env.current.direction.y, env.current.direction.x);
                }
            }
        ...
        const currentTarget = this.state === 'deploying' ? this.target : this.startPos;
        const distToTarget = Math.hypot(currentTarget.x - this.pos.x, currentTarget.y - this.pos.y);
        ...
        let desiredAngle = Math.atan2(currentTarget.y - this.pos.y, currentTarget.x - this.pos.x);
        ...
        if (actualLookAhead > 0) {
            const anglesToCheck = [0, -Math.PI/4, Math.PI/4, -Math.PI/2, Math.PI/2]; 
            let clearAngle = null;
