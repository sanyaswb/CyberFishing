...
let sawComposedHoldFromPointer = false;
let lastDebug = null;
...
for (let frame = 0; frame < 240; frame++) {
  ...
  sawComposedHoldFromPointer ||= lastDebug.rodPullActive === true;
...
assert(sawComposedHoldFromPointer, "pointer-only raw input is composed into Fight Rod Hold");
assert(sawHold, "composed Fight Rod Hold produces pull force");
assert(sawControl, "horizontal pointer movement composes Rod Control during hold");
...
assert(
  ...
  "0.05 kg fish reaches victory with pointer hold + lateral control"
