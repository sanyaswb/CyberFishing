...
const { TRANSITION } = require("./domain_batches/stage_three_batch_014_release_transition");
...
const OUTPUT = "architecture/migration/stage_3_batch_014_release_regression.json";

class Batch014ReleaseRegression {
  run(root = path.resolve(__dirname, "../..")) {
    assert(!fs.existsSync(path.join(root, OUTPUT)), "Release regression attempt is immutable");
    const release = new Batch014ReleaseCheck().run(root);
    const transitionSha256 = sha(fs.readFileSync(path.join(root, TRANSITION)));
    const automatedPath = "architecture/migration/stage_3_batch_014_automated_acceptance.json";
    ...
    const acceptance = JSON.parse(fs.readFileSync(path.join(root,
      "architecture/migration/stage_3_batch_014_acceptance_pass.json")));
    ...
    const report = {
      schemaVersion: 1, kind: "cyber-fishing-stage-3-batch-014-release-regression",
      batchId: "stage-3.candidate-014-fishing-fefd469b",
      releaseVersion: "0.24.51", recordedAt: new Date().toISOString(), status: "passed",
    ...
    console.log("Stage 3.14.9 regression PASS: release metadata exact; pre-release npm ci/build retained; generated runtime byte-identical.");
    return report;
  }
}

if (require.main === module) {
  try { new Batch014ReleaseRegression().run(); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch014ReleaseRegression, OUTPUT };
