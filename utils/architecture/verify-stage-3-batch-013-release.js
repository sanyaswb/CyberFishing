...
const OUTPUT = "architecture/migration/stage_3_batch_013_release_regression.json";

class Batch013ReleaseRegression {
  run(root = path.resolve(__dirname, "../..")) {
    assert(!fs.existsSync(path.join(root, OUTPUT)), "Release regression attempt is immutable");
    const release = new Batch013ReleaseCheck().run(root);
    const transitionSha256 = sha(fs.readFileSync(path.join(root, TRANSITION)));
    const automatedPath = "architecture/migration/stage_3_batch_013_automated_acceptance.json";
    ...
    const acceptance = JSON.parse(fs.readFileSync(path.join(root,
      "architecture/migration/stage_3_batch_013_acceptance_pass.json")));
    ...
    const report = {
      schemaVersion: 1, kind: "cyber-fishing-stage-3-batch-013-release-regression",
      batchId: "stage-3.candidate-013-fishing-0267b3ea",
      releaseVersion: "0.24.50", recordedAt: new Date().toISOString(), status: "passed",
    ...
    console.log("Stage 3.13.9 regression PASS: release metadata exact; pre-release npm ci/build retained; generated runtime byte-identical.");
    return report;
  }
}

if (require.main === module) {
  try { new Batch013ReleaseRegression().run(); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch013ReleaseRegression, OUTPUT };
