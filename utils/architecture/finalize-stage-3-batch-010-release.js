...
const { TRANSITION, RELEASE_PATHS } = require("./domain_batches/stage_three_batch_010_release_transition");
...
const OUTPUT = "architecture/migration/stage_3_batch_010_release_closure.json";
const REGRESSION = "architecture/migration/stage_3_batch_010_release_regression.json";

class Batch010ReleaseFinalizer {
  run(root = path.resolve(__dirname, "../..")) {
    ...
    const verified = new Batch010ReleaseCheck().run(root);
    ...
    const planPath = "architecture/migration/stage_3_batch_010_execution_plan.json";
    ...
    assert.equal(regression.releaseVersion, "0.24.47");
    ...
    for (const [id, suite] of [["architecture", "architecture"], ["quick", "quick"], ["full", "all"]]) {
      ...
      assert.equal(step.passedChecks,
    ...
    const closure = {
      ...
      releaseVersion: "0.24.47", previousReleaseVersion: "0.24.46",
      releasePublicationAllowed: true, completedBatchCount: 10,
      completedDomainModuleCount: approved.batches.slice(0, 10)
        .reduce((count, batch) => count + batch.modules.length, 0),
      runtimeTopology: verified.topology,
      acceptance: {
        automatedEvidence: reference("architecture/migration/stage_3_batch_010_automated_acceptance.json"),
        historicalSummary: reference("architecture/migration/stage_3_batch_010_acceptance_pass.json"),
        browserProof: reference("architecture/migration/stage_3_batch_010_browser_confirmation.json"),
      ...
      nextStage: "Stage 3.11.0 — Batch 011 Preflight and Dependency/State Audit",
    };
    assert.equal(closure.completedDomainModuleCount, 28);
    ...
    console.log("Stage 3.10.9 release closure verified: v0.24.47, ten completed batches, 28 frozen Domain modules, exact batch-only rollback.");
    return closure;
  }
}

if (require.main === module) {
  try { new Batch010ReleaseFinalizer().run(); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch010ReleaseFinalizer, OUTPUT };
