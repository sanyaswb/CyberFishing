...
const AUTOMATED = "architecture/migration/stage_3_batch_010_automated_acceptance.json";
const ACCEPTANCE = "architecture/migration/stage_3_batch_010_acceptance_pass.json";
const BROWSER = "architecture/migration/stage_3_batch_010_browser_confirmation.json";

class Batch010BrowserAcceptanceRecorder {
  ...
  run({ sessionStatement, consoleStatement, errors, warnings }) {
    ...
    assert(sessionStatement.includes("PASS") && sessionStatement.includes("ловлі риби"));
    ...
    const acceptance = {
      schemaVersion: 1, kind: "cyber-fishing-stage-3-batch-010-acceptance",
      ...
      browserSmoke: {
        ...
        context: "Response to the Stage 3.10.8 checklist: game load, Canvas, inventory assembly creation/component and full equipment-to-fish session.",
        acceptanceBasis: "Collective PASS for the checklist and full game session; individual steps were not independently instrumented.",
      ...
      nextStage: "Stage 3.10.9 — Release Closure v0.24.47",
    };
    const acceptanceBytes = serialize(acceptance);
    const browser = {
      schemaVersion: 1, kind: "cyber-fishing-stage-3-batch-010-browser-confirmation",
      ...
      browserSummary: {
        userStatement: sessionStatement,
        context: "Collective PASS for the Stage 3.10.8 checklist and full equipment-to-fish gameplay cycle.",
...
module.exports = { Batch010BrowserAcceptanceRecorder, AUTOMATED, ACCEPTANCE, BROWSER };
