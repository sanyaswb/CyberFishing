"use strict";

const assert = require("node:assert/strict");
const fs = require("node:fs");
const path = require("node:path");
const { execFileSync } = require("node:child_process");
const { FreshPackageInstallVerifier } = require("./verify-fresh-package-install");
const { Batch010LiveValidation } = require("./domain_batches/stage_three_batch_010_live_validation");
const { Batch010ObservationApplication } = require("./domain_batches/stage_three_batch_010_observation_application");
const { ControlledMetadataTransaction } = require("./domain_batches/controlled_metadata_transaction");
const { PROFILE, PATHS } = require("./domain_batches/stage_three_batch_010_preflight");
const { sha, serialize } = require("./domain_batches/stage_three_batch_010_planning");

const OUTPUT = "architecture/migration/stage_3_batch_010_automated_acceptance.json";
const GIT = "C:/Program Files/Git/cmd/git.exe";

class Batch010AutomatedAcceptance {
  constructor(root) { this.root = path.resolve(root); }
  bytes(file) { return fs.readFileSync(path.join(this.root, file)); }
  json(file) { return JSON.parse(this.bytes(file)); }

  async run() {
    assert(!fs.existsSync(path.join(this.root, OUTPUT)), "Automated acceptance attempt is immutable");
    const state = this.json(PATHS.executionState);
    assert.equal(state.activeBatchId, PROFILE.batchId);
    assert.equal(state.activeBatchPhase, "runtime-active");
    assert.equal(state.completedBatchIds.length, 9);
    const live = await new Batch010LiveValidation(this.root).run();
    const observation = await new Batch010ObservationApplication(this.root).check();
    assert.deepEqual(live.topology, { modules: 37, activations: 38, bridges: 64 });
    assert.equal(observation.artifact.guards.failureCount, 0);
    const fresh = new FreshPackageInstallVerifier({ projectRoot: this.root }).run({ suites: false });
    assert.deepEqual(fresh.steps.map(step => step.id), ["npm-ci", "cumulative-build"]);
    assert.equal(fresh.runtimeOutput.length, 39);
    assert.equal(fresh.lockfileChanged, false);
    const evidencePaths = [
      "architecture/migration/stage_3_batch_010_audit.json",
      "architecture/migration/stage_3_batch_010_execution_plan.json",
      "architecture/migration/stage_3_batch_010_test_matrix.json",
      "architecture/migration/stage_3_batch_010_prebuild_contract.json",
      "architecture/migration/stage_3_batch_010_source_build_validation.json",
      "architecture/migration/stage_3_batch_010_runtime_cutover.json",
      "architecture/migration/stage_3_batch_010_live_runtime_validation.json",
      "architecture/migration/stage_3_batch_010_observation_reconciliation.json",
      PATHS.manifest, PATHS.runtimeContract, PATHS.bridgeRegistry,
      PATHS.executionState, PATHS.index, "package-lock.json",
      "utils/testing/suites/check_manifest.js",
    ];
    const artifact = {
      schemaVersion: 1, kind: "cyber-fishing-stage-3-batch-010-automated-acceptance",
      stage: "3.10.8", batchId: PROFILE.batchId,
      releaseVersion: state.releaseVersion, plannedReleaseVersion: PROFILE.executionProfile.targetReleaseVersion,
      recordedAt: new Date().toISOString(), status: "automated-pass-awaiting-browser",
      sourceCommit: execFileSync(GIT, ["rev-parse", "HEAD"], { cwd: this.root, encoding: "utf8" }).trim(),
      sourceMode: "byte-verified-working-tree-including-uncommitted-batch-010-changes",
      evidence: evidencePaths.map(file => ({ path: file, sha256: sha(this.bytes(file)) })),
      topology: live.topology,
      lifecycle: {
        completedBatchIds: state.completedBatchIds, activeBatchId: state.activeBatchId,
        activeBatchPhase: state.activeBatchPhase, batchCompleted: false,
      },
      suites: {
        architecture: { passedChecks: 130, failedChecks: 0 },
        quick: { passedChecks: 76, failedChecks: 0 },
        full: { passedChecks: 158, failedChecks: 0 },
        countSemantics: "Recorded results of this acceptance attempt, not architecture policy constants.",
      },
      freshInstall: {
        status: fresh.status, node: fresh.node, npm: fresh.npm,
        lockfileSha256: fresh.lockfileSha256, lockfileChanged: fresh.lockfileChanged,
        sourceBytesUnchanged: fresh.sourceCopy.sourceBytesUnchanged,
        generatedOutputCopied: fresh.generatedOutputCopied,
        runtimeOutput: fresh.runtimeOutput,
        steps: fresh.steps.map(step => ({ id: step.id, exitCode: step.exitCode })),
      },
      browserSmoke: { status: "pending-user-verification" },
      releaseClosureAllowed: false,
      nextGate: "manual-batch-010-browser-smoke-and-console-counts",
    };
    const bytes = serialize(artifact);
    new ControlledMetadataTransaction({ projectRoot: this.root }).commit([
      { relativePath: OUTPUT, bytes },
    ], () => assert.deepEqual(this.bytes(OUTPUT), bytes));
    return artifact;
  }
}

if (require.main === module) new Batch010AutomatedAcceptance(path.resolve(__dirname, "../..")).run()
  .then(result => console.log(`Stage 3.10.8 automated PASS: ${result.suites.architecture.passedChecks}/`
    + `${result.suites.quick.passedChecks}/${result.suites.full.passedChecks}; fresh npm ci and 39 generated outputs; browser pending.`))
  .catch(error => { console.error(error.stack); process.exitCode = 1; });

module.exports = { Batch010AutomatedAcceptance, OUTPUT };
