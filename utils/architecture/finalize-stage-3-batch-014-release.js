...
const { Batch014ReleaseCheck } = require("./stage-3-batch-014-release-check");
const { ControlledMetadataTransaction } = require("./domain_batches/controlled_metadata_transaction");
const { TRANSITION, RELEASE_PATHS } = require("./domain_batches/stage_three_batch_014_release_transition");
...
const OUTPUT = "architecture/migration/stage_3_batch_014_release_closure.json";
const REGRESSION = "architecture/migration/stage_3_batch_014_release_regression.json";

class Batch014ReleaseFinalizer {
  run(root = path.resolve(__dirname, "../..")) {
    ...
    const verified = new Batch014ReleaseCheck().run(root);
    const accepted = json("architecture/migration/stage_3_batch_014_acceptance_pass.json");
    const browser = json("architecture/migration/stage_3_batch_014_browser_confirmation.json");
    const regression = json(REGRESSION);
    const transition = json(TRANSITION);
    const planPath = "architecture/migration/stage_3_batch_014_execution_plan.json";
    ...
    assert.equal(browser.supplements.sha256,
      sha(read("architecture/migration/stage_3_batch_014_acceptance_pass.json")));
    assert.deepEqual([browser.console.errors, browser.console.warnings], [0, 0]);
    assert.equal(regression.status, "passed");
    assert.equal(regression.releaseVersion, "0.24.51");
    ...
    assert.equal(regression.automatedAcceptance.sha256,
      sha(read("architecture/migration/stage_3_batch_014_automated_acceptance.json")));
    ...
    const closure = {
      ...
      releaseVersion: "0.24.51", previousReleaseVersion: "0.24.50",
      releasePublicationAllowed: true, completedBatchCount: 14,
      completedDomainModuleCount: approved.batches.slice(0, 14)
        .reduce((count, batch) => count + batch.modules.length, 0),
      runtimeTopology: verified.topology,
      acceptance: {
        automatedEvidence: reference("architecture/migration/stage_3_batch_014_automated_acceptance.json"),
        historicalSummary: reference("architecture/migration/stage_3_batch_014_acceptance_pass.json"),
        browserProof: reference("architecture/migration/stage_3_batch_014_browser_confirmation.json"),
      ...
      nextStage: "Stage 3.15.0 — Batch 015 Preflight and Dependency/State Audit",
    };
    assert.equal(closure.completedDomainModuleCount, 47);
    ...
    new Batch014ReleaseCheck().run(root);
    console.log("Stage 3.14.9 release closure verified: v0.24.51, fourteen completed batches, 41 frozen Domain modules, exact batch-only rollback.");
    return closure;
  }
}

if (require.main === module) {
  try { new Batch014ReleaseFinalizer().run(); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch014ReleaseFinalizer, OUTPUT };
