...
class Batch013IsolatedSuite {
  run(root = path.resolve(__dirname, "../.."), suite = "all", startIndex = 0) {
    const parent = fs.realpathSync(os.tmpdir());
    const temporary = fs.mkdtempSync(path.join(parent, "cyber-batch013-suite-"));
    ...
    try {
      ...
      assert(path.basename(resolved).startsWith("cyber-batch013-suite-"));
...
if (require.main === module) {
  try { new Batch013IsolatedSuite().run(undefined, process.argv[2] || "all",
    Number(process.argv[3] || 0)); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch013IsolatedSuite };
