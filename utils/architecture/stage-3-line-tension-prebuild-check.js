...
const CONSUMER = "src/systems/tension_system.js";
const LEGACY_SYMBOL = "LineTensionCalculator";
const ACTIVATION_ID = "activation-a8e624a68a8e";

class StageThreeLineTensionPrebuildCheck {
  run() {
    ...
    assert.deepEqual(state.completedBatchIds.slice(0, 2), [
    ...
    assert.equal(approvedActivation.contract.legacyScriptIndex, 129);
    ...
    assert.deepEqual(records[0], {
      ...
      reason: "Preserve the exact synchronous TensionSystem consumer until application migration removes the legacy symbol.",
      owner: BATCH_ID,
    ...
    console.log(
      "Stage 3.3 pre-build contract passed: frozen batch, exact ESM target, registry consumer set, activation-129 contract and bridge-only source verified.",
    );
  }
...
new StageThreeLineTensionPrebuildCheck().run();
