...
const { Batch011Planning, PLAN_PATHS, serialize } = require("./domain_batches/stage_three_batch_011_planning");
const { RepositoryContentSnapshot } = require("./esm_infrastructure/repository_content_snapshot");

class Batch011PlanningCheck {
  run(root = path.resolve(__dirname, "../..")) {
    const planning = new Batch011Planning(root);
    ...
    try {
      ...
      assert.equal(matrix.scope.moduleCount, 5);
      console.log(`Stage 3.11.1–.2 PASS: ${plan.scope.targetCount} targets, ${plan.scope.consumerRelationshipCount} bridges, ${matrix.behaviorCases.length} behavior cases; batch-only rollback and runtime unchanged.`);
      return { plan, matrix };
    } finally {
...
if (require.main === module) {
  new (require("./domain_batches/stage_three_batch_011_historical_workspace").Batch011HistoricalWorkspace)()
    .run(path.resolve(__dirname, "../.."), root => new Batch011PlanningCheck().run(root))
    .catch(error => { console.error(error.stack); process.exitCode = 1; });
}

module.exports = { Batch011PlanningCheck };
