...
const { Batch012LiveValidation } = require("./stage_three_batch_012_live_validation");
const { Batch012CutoverHistory } = require("./stage_three_batch_012_cutover_history");
const { Batch012ObservationReconciliation, Batch012ObservationContract } = require("./stage_three_batch_012_observation_reconciliation");
const { beforeBatch012Observations, MANIFEST, OUTPUT, PREBUILD, CUTOVER, RUNTIME, KIND, NEXT } = require("./stage_three_batch_012_observation_transition");

const PATHS = Object.freeze({
  prebuild: PREBUILD, cutover: CUTOVER, runtime: RUNTIME,
  live: "architecture/migration/stage_3_batch_012_live_runtime_validation.json",
  audit: "architecture/migration/stage_3_batch_012_audit.json",
...
class Batch012ObservationApplication {
  ...
  async prepare() {
    ...
    new Batch012CutoverHistory(this.root).artifact();
    await new Batch012LiveValidation(this.root).run();
    const data = Object.fromEntries(Object.entries(PATHS).map(([key, file]) => [key, this.json(file)]));
    const pendingBytes = beforeBatch012Observations(this.bytes(MANIFEST), this.root);
    ...
    const result = new Batch012ObservationReconciliation().build(inputs);
    ...
    new Batch012ObservationContract(data.prebuild).validate(artifact, artifact);
  ...
  async check() {
    ...
    new Batch012ObservationContract(result.inputs.prebuild).validate(this.json(OUTPUT), result.artifact);
  ...
  async run() {
    ...
    new ControlledMetadataTransaction({ projectRoot: this.root, failureInjector: this.failureInjector }).commit(writes, () => {
      ...
      assert.deepEqual(beforeBatch012Observations(this.bytes(MANIFEST), this.root), canonicalBytes(result.inputs.before));
...
module.exports = { Batch012ObservationApplication, PATHS };
