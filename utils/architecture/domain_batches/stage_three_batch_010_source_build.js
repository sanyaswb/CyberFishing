...
const { Batch010CandidateWorkspace } = require("./stage_three_batch_010_candidate_workspace");
...
const OUTPUT = "architecture/migration/stage_3_batch_010_source_build_validation.json";

class Batch010SourceBuild {
  constructor(root) {
    this.root = path.resolve(root);
    this.app = new Batch010Planning(this.root);
    this.prebuild = new Batch010Prebuild(this.root);
  }
  ...
  snapshot() {
    const paths = [PATHS.executionState, PATHS.manifest, PATHS.runtimeContract,
      ...
      PROFILE.executionProfile.expectedTargets[0].currentPath,
  ...
  async candidate({ instrumented = false, failure = null, mutateOutput = output => output,
    ...
    try {
      ...
      const viteLoader = async () => {
        ...
        return { build: async options => {
          ...
          return vite.build({ ...options, plugins: [...options.plugins, {
            name: "batch010-test-only-evaluation-counter",
            transform(code, id) {
      ...
      const report = await new StageThreeBatchCandidateBuild({
        projectRoot: workspace.root, batchNumber: "010", additionalVirtualModules: [], viteLoader,
        verifyOutput: output => {
          if (failure === "output") throw new Error("injected candidate output rejection");
          validation = new Batch010CandidateValidation().run({ ...mutateOutput(output), app: this.app, instrumented });
  ...
  async prepare() {
    ...
    return {
      ...
      nextGate: "stage-3.10.5-atomic-runtime-cutover",
...
module.exports = { Batch010SourceBuild, OUTPUT };
