...
const { Batch014LiveValidation } = require("./stage_three_batch_014_live_validation");
const { Batch014CutoverHistory } = require("./stage_three_batch_014_cutover_history");
const { Batch014ObservationReconciliation, Batch014ObservationContract } = require("./stage_three_batch_014_observation_reconciliation");
const { beforeBatch014Observations, MANIFEST, OUTPUT, PREBUILD, CUTOVER, RUNTIME, KIND, NEXT } = require("./stage_three_batch_014_observation_transition");

const PATHS = Object.freeze({
  prebuild: PREBUILD, cutover: CUTOVER, runtime: RUNTIME,
  live: "architecture/migration/stage_3_batch_014_live_runtime_validation.json",
  audit: "architecture/migration/stage_3_batch_014_audit.json",
...
class Batch014ObservationApplication {
  ...
  async prepare() {
    ...
    new Batch014CutoverHistory(this.root).artifact();
    await new Batch014LiveValidation(this.root).run();
    const data = Object.fromEntries(Object.entries(PATHS).map(([key, file]) => [key, this.json(file)]));
    const pendingBytes = beforeBatch014Observations(this.bytes(MANIFEST), this.root);
    ...
    const result = new Batch014ObservationReconciliation().build(inputs);
    ...
    new Batch014ObservationContract(data.prebuild).validate(artifact, artifact);
  ...
  async check() {
    ...
    new Batch014ObservationContract(result.inputs.prebuild).validate(this.json(OUTPUT), result.artifact);
  ...
  async run() {
    ...
    new ControlledMetadataTransaction({ projectRoot: this.root, failureInjector: this.failureInjector }).commit(writes, () => {
      ...
      assert.deepEqual(beforeBatch014Observations(this.bytes(MANIFEST), this.root), canonicalBytes(result.inputs.before));
...
module.exports = { Batch014ObservationApplication, PATHS };
