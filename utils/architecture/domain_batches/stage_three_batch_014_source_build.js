...
const { Batch014CandidateWorkspace } = require("./stage_three_batch_014_candidate_workspace");
...
const OUTPUT = "architecture/migration/stage_3_batch_014_source_build_validation.json";

class Batch014SourceBuild {
  constructor(root) {
    this.root = path.resolve(root);
    this.app = new Batch014Planning(this.root);
    this.prebuild = new Batch014Prebuild(this.root);
  }
  ...
  async candidate({ instrumented = false, failure = null, mutateOutput = output => output,
    ...
    const workspace = new Batch014CandidateWorkspace();
    try {
      ...
      const viteLoader = async () => {
        ...
        return { build: async options => {
          ...
          return vite.build({ ...options, plugins: [...options.plugins, {
            name: "batch014-test-only-evaluation-counter",
            transform(code, id) {
      ...
      const report = await new StageThreeBatchCandidateBuild({
        projectRoot: workspace.root, batchNumber: "014", additionalVirtualModules: [],
        ...
        verifyOutput: output => {
          ...
          validation = new Batch014CandidateValidation().run({ ...mutateOutput(output), app: this.app, instrumented });
  ...
  async prepare() {
    ...
    return {
      ...
      nextGate: "stage-3.14.5-atomic-runtime-cutover",
...
module.exports = { Batch014SourceBuild, OUTPUT };
