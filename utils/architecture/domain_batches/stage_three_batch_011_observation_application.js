...
const { Batch011LiveValidation } = require("./stage_three_batch_011_live_validation");
const { Batch011CutoverHistory } = require("./stage_three_batch_011_cutover_history");
const { Batch011ObservationReconciliation, Batch011ObservationContract } = require("./stage_three_batch_011_observation_reconciliation");
const { beforeBatch011Observations, MANIFEST, OUTPUT, PREBUILD, CUTOVER, RUNTIME, KIND, NEXT } = require("./stage_three_batch_011_observation_transition");

const PATHS = Object.freeze({
  prebuild: PREBUILD, cutover: CUTOVER, runtime: RUNTIME,
  live: "architecture/migration/stage_3_batch_011_live_runtime_validation.json",
  audit: "architecture/migration/stage_3_batch_011_audit.json",
...
class Batch011ObservationApplication {
  ...
  async prepare() {
    ...
    new Batch011CutoverHistory(this.root).artifact();
    await new Batch011LiveValidation(this.root).run();
    const data = Object.fromEntries(Object.entries(PATHS).map(([key, file]) => [key, this.json(file)]));
    const pendingBytes = beforeBatch011Observations(this.bytes(MANIFEST), this.root);
    ...
    const result = new Batch011ObservationReconciliation().build(inputs);
    ...
    new Batch011ObservationContract(data.prebuild).validate(artifact, artifact);
  ...
  async check() {
    ...
    new Batch011ObservationContract(result.inputs.prebuild).validate(this.json(OUTPUT), result.artifact);
  ...
  async run() {
    ...
    new ControlledMetadataTransaction({ projectRoot: this.root, failureInjector: this.failureInjector }).commit(writes, () => {
      ...
      assert.deepEqual(beforeBatch011Observations(this.bytes(MANIFEST), this.root), canonicalBytes(result.inputs.before));
...
module.exports = { Batch011ObservationApplication, PATHS };
