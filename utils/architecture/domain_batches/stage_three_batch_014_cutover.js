...
const { Batch014SourceBuild, OUTPUT: SOURCE_BUILD } = require("./stage_three_batch_014_source_build");
const { Batch014Prebuild, PREBUILD } = require("./stage_three_batch_014_prebuild");
...
const { sha, serialize } = require("./stage_three_batch_014_planning");
...
class Batch014CutoverProjection {
  async prepare(root) {
    const prebuild = new Batch014Prebuild(root).validateOpen();
    const builder = new Batch014SourceBuild(root);
    ...
    packageContract.stage.current = "3.14";
...
class Batch014AtomicCutover {
  ...
  async run() {
    assert(!fs.existsSync(path.join(this.root, CUTOVER)), "Batch 014 cutover is already published");
    const prepared = await new Batch014CutoverProjection().prepare(this.root);
    return new Batch014PublicationGate().run(() => {
      new Batch014Prebuild(this.root).validateOpen();
...
module.exports = { Batch014CutoverProjection, Batch014AtomicCutover, Batch014PublicationGate, CUTOVER };
