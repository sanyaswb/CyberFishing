...
class StageThreeBatchCandidateBuild {
  ...
  async run({ prebuild, approvedPlan, executionState, runtimeContract,
    ...
    try {
      ...
      if (this.verifyOutput) await this.verifyOutput({ report: result, contract: futureContract,
        readOutput: (outputPath) => fs.readFileSync(this.outputManager.candidatePath(outputPath), "utf8") });
  ...
  #validateReport(report, prebuild) {
    ...
    for (const helper of this.additionalVirtualModules) {
  ...
  #absolute(relativePath) {
    ...
    if (relative !== relativePath) {
      throw new Error(`Stage 3 candidate path escaped project root: ${relativePath}`);
    }
    return absolute;
  }
...
module.exports = { StageThreeBatchCandidateBuild };
