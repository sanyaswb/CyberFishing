...
const { Batch010CutoverProjection, CUTOVER } = require("./stage_three_batch_010_cutover");
const { Batch010HistoricalWorkspace } = require("./stage_three_batch_010_historical_workspace");
const { Batch010CandidateValidation } = require("./stage_three_batch_010_candidate_validation");
const { Batch010Planning, sha, serialize } = require("./stage_three_batch_010_planning");
const { PATHS, PROFILE } = require("./stage_three_batch_010_preflight");
...
const OUTPUT = "architecture/migration/stage_3_batch_010_live_runtime_validation.json";

class Batch010LiveValidation {
  ...
  verifyPublished() {
    ...
    for (const write of cutover.writes) {
      ...
      const actual = write.path === PATHS.manifest
        ? require("./stage_three_batch_010_observation_transition").beforeBatch010Observations(this.bytes(write.path), this.root)
    ...
    const oldSourceWrite = cutover.writes.find(write =>
      write.path === PROFILE.executionProfile.expectedTargets[0].currentPath);
    assert(oldRuntimeWrite && oldCodeWrite && oldSourceWrite);
    ...
    const classicSource = Buffer.from(oldSourceWrite.beforeBase64, "base64").toString("utf8");
    const app = new Batch010Planning(this.root);
    const outputValidation = new Batch010CandidateValidation().run({
      app, report: proof.report, contract: runtime, readOutput: file => this.bytes(file).toString("utf8"),
      classicSource, previousRuntimeContract: oldContract, previousRuntimeCode: oldCode,
    });
    ...
    for (const source of proof.sources) {
      ...
      for (const test of Object.values(BATCH_010_EXECUTABLE_CASES[source.exportName])) test(Type);
    ...
    return {
      ...
      nextGate: "stage-3.10.7-observation-reconciliation",
    };
  }

  async run({ persist = false } = {}) {
    const cutover = this.json(CUTOVER);
    const replay = await new Batch010HistoricalWorkspace().run(this.root,
      root => new Batch010CutoverProjection().prepare(root), { keepPrebuild: true });
...
module.exports = { Batch010LiveValidation, OUTPUT };
