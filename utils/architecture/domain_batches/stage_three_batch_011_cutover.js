...
const { Batch011SourceBuild, OUTPUT: SOURCE_BUILD } = require("./stage_three_batch_011_source_build");
...
const { sha, serialize } = require("./stage_three_batch_011_planning");
...
class Batch011CutoverProjection {
  async prepare(root) {
    const prebuild = new Batch011Prebuild(root).validateOpen();
    ...
    packageContract.stage.current = "3.11";
...
class Batch011AtomicCutover {
      ...
      for (const write of prepared.writes) {
        ...
        if (!fs.existsSync(parent)) {
          ...
          for (let directory = parent; !fs.existsSync(directory); directory = path.dirname(directory)) {
            ...
            assert(["src/game/domain/items", "src/game/domain/items/progression",
              "src/game/domain/items/rarity"].includes(relative), `Unexpected target directory: ${relative}`);
  ...
  async run() {
    assert(!fs.existsSync(path.join(this.root, CUTOVER)), "Batch 011 cutover is already published");
    const prepared = await new Batch011CutoverProjection().prepare(this.root);
    return new Batch011PublicationGate().run(() => {
      new Batch011Prebuild(this.root).validateOpen();
...
module.exports = { Batch011CutoverProjection, Batch011AtomicCutover, Batch011PublicationGate, CUTOVER };
