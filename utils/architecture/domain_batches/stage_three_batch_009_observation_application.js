...
const { Batch009LiveValidation } = require("./stage_three_batch_009_live_validation");
const { Batch009CutoverHistory } = require("./stage_three_batch_009_cutover_history");
const { Batch009ObservationReconciliation, Batch009ObservationContract } = require("./stage_three_batch_009_observation_reconciliation");
const { beforeBatch009Observations, MANIFEST, OUTPUT, PREBUILD, CUTOVER, RUNTIME, KIND, NEXT } = require("./stage_three_batch_009_observation_transition");

const PATHS = Object.freeze({
  prebuild: PREBUILD, cutover: CUTOVER, runtime: RUNTIME,
  live: "architecture/migration/stage_3_batch_009_live_runtime_validation.json",
  audit: "architecture/migration/stage_3_batch_009_audit.json",
...
class Batch009ObservationApplication {
  ...
  async prepare() {
    ...
    new Batch009CutoverHistory(this.root).artifact();
    await new Batch009LiveValidation(this.root).run();
    const data = Object.fromEntries(Object.entries(PATHS).map(([key, file]) => [key, this.json(file)]));
    const pendingBytes = beforeBatch009Observations(this.bytes(MANIFEST), this.root);
    ...
    const result = new Batch009ObservationReconciliation().build(inputs);
    ...
    const reverseConsumers = data.prebuild.preliminaryMetadata.plannedActivationPositions.map(activation => {
      const legacy = reverse.consumersOf(activation.sourceProvider);
      const consumerPaths = data.registry.bridges.filter(bridge => bridge.bridge === activation.sourceProvider).map(bridge => bridge.source);
      assert.deepEqual(legacy, consumerPaths.map(source => ({ source, symbols: [activation.legacySymbol] }))
    ...
    new Batch009ObservationContract(data.prebuild).validate(artifact, artifact);
  ...
  async check() {
    ...
    new Batch009ObservationContract(result.inputs.prebuild).validate(this.json(OUTPUT), result.artifact);
  ...
  async run() {
    ...
    new ControlledMetadataTransaction({ projectRoot: this.root, failureInjector: this.failureInjector }).commit(writes, () => {
      ...
      assert.deepEqual(beforeBatch009Observations(this.bytes(MANIFEST), this.root), canonicalBytes(result.inputs.before));
...
module.exports = { Batch009ObservationApplication, PATHS };
