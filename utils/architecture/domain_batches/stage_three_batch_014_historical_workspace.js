...
const OBSERVATION = "architecture/migration/stage_3_batch_014_observation_reconciliation.json";
const MANIFEST = "architecture/migration/module_migration_manifest.json";
const RELEASE = "architecture/migration/stage_3_batch_014_release_transition.json";
const sha = bytes => crypto.createHash("sha256").update(bytes).digest("hex");

class Batch014HistoricalWorkspace {
  async run(root, action, { keepPrebuild = false, keepCutover = false, copyTools = false } = {}) {
    ...
    const temporary = fs.mkdtempSync(path.join(parent, "cyber-batch014-historical-"));
    const copy = relative => {
    ...
    try {
      ...
      if (fs.existsSync(releasePath)) {
        const transition = new (require("./stage_three_batch_014_release_transition").Batch014ReleaseTransition)(temporary);
      ...
      if (fs.existsSync(observationPath)) {
        ...
        const prior = require("./stage_three_batch_014_observation_transition")
          .beforeBatch014Observations(fs.readFileSync(manifest), temporary);
      ...
      if (fs.existsSync(cutoverPath)) {
        ...
        for (const record of cutover.writes) {
          ...
          assert.equal(current && sha(current), record.afterSha256, `batch 014 cutover drift: ${record.path}`);
      ...
      assert(path.basename(resolved).startsWith("cyber-batch014-historical-"));
      fs.rmSync(resolved, { recursive: true, force: true });
    }
  }
}

module.exports = { Batch014HistoricalWorkspace };
