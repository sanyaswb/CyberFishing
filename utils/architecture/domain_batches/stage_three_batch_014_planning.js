...
const { BATCH_014_MATRIX_DEPENDENCIES } = require("./stage_three_batch_014_focused_test_catalog");
...
class Batch014Planning {
  ...
  matrix() {
    ...
    const matrix = new StageThreeBatchFocusedTestMatrixBuilder(BATCH_014_MATRIX_DEPENDENCIES)
      .build({ executionPlan: plan,
        executionPlanSha256: crypto.createHash("sha256").update(bytes).digest("hex") });
    return new StageThreeBatchFocusedTestMatrixValidator(BATCH_014_MATRIX_DEPENDENCIES)
      .validate(matrix);
  }
}
...
module.exports = { Batch014Planning, PLAN_PATHS, ROLLBACK_PATHS, sha, serialize };
