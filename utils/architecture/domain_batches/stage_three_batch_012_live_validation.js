...
const { Batch012CutoverProjection, CUTOVER } = require("./stage_three_batch_012_cutover");
const { Batch012HistoricalWorkspace } = require("./stage_three_batch_012_historical_workspace");
const { Batch012CandidateValidation } = require("./stage_three_batch_012_candidate_validation");
const { Batch012Planning, sha, serialize } = require("./stage_three_batch_012_planning");
...
const OUTPUT = "architecture/migration/stage_3_batch_012_live_runtime_validation.json";

class Batch012LiveValidation {
  ...
  verifyPublished() {
    ...
    for (const write of cutover.writes) {
      ...
      const actual = write.path === PATHS.manifest && observationPublished
        ? require("./stage_three_batch_012_observation_transition").beforeBatch012Observations(this.bytes(write.path), this.root)
    ...
    const app = new Batch012Planning(this.root);
    const outputValidation = new Batch012CandidateValidation().run({
    ...
    for (const source of proof.sources) {
      ...
      new RepresentationOnlyNamedEsmTarget().validate({
        ...
        legacyExposure: PROFILE.reviewedContracts[source.currentPath].legacyExposure,
      });
      ...
      assert.deepEqual(after.topLevelEffects, [], "ESM target retained a top-level effect");
      assert.deepEqual(before.topLevelEffects,
        [`ExpressionStatement@${PROFILE.reviewedContracts[source.currentPath].legacyExposure.location}`]);
      const Type = activation.transport.modules[source.targetPath][source.exportName];
      for (const test of Object.values(BATCH_012_EXECUTABLE_CASES[source.exportName])) test(Type);
    ...
    return {
      ...
      nextGate: "stage-3.12.7-observation-reconciliation",
    };
  }

  async run({ persist = false } = {}) {
    const cutover = this.json(CUTOVER);
    const replay = await new Batch012HistoricalWorkspace().run(this.root,
      root => new Batch012CutoverProjection().prepare(root), { keepPrebuild: true });
...
module.exports = { Batch012LiveValidation, OUTPUT };
