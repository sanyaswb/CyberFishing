...
const { sha } = require("./stage_three_batch_011_planning");
...
const { CUTOVER } = require("./stage_three_batch_011_cutover");
const { MANIFEST, beforeBatch011Observations } = require("./stage_three_batch_011_observation_transition");
const { Batch011ReleaseTransition } = require("./stage_three_batch_011_release_transition");

const STATE = "architecture/migration/stage_3_execution_state.json";

class Batch011History {
  ...
  before(file, provided = this.bytes(file)) {
    let bytes = new (require("./stage_three_batch_012_history").Batch012History)(this.root)
      .before(file, provided);
    if (!this.exists()) return bytes;
    bytes = new Batch011ReleaseTransition(this.root).before(file, bytes);
    if (file === MANIFEST) bytes = beforeBatch011Observations(bytes, this.root);
    const prebuild = JSON.parse(this.bytes(PREBUILD));
    if (fs.existsSync(path.join(this.root, CUTOVER))) {
      const cutover = new Batch011CutoverHistory(this.root).artifact();
...
module.exports = { Batch011History };
