...
const { Batch012Planning, PLAN_PATHS } = require("./stage_three_batch_012_planning");
...
class Batch012Prebuild {
  constructor(root) {
    this.root = path.resolve(root);
    this.planning = new Batch012Planning(this.root);
    const approved = this.planning.json(PATHS.approvedPlan);
    const plan = this.planning.json(PLAN_PATHS.output);
    const completedPrefix = approved.batches.slice(0, 11).map(batch => batch.id);
    this.profile = new StageThreeBatchPrebuildProfile({
      schemaVersion: 1, contractSchemaVersion: 2, stageLabel: "Stage 3.12.3",
      ...
      unlockCondition: "stage-3.12.4-target-source-and-build-validation",
  ...
  build() {
    ...
    const artifact = {
      ...
      publication: {
        ...
        rollback: "restore-exact-before-state-and-remove-only-batch-012-prebuild-contract",
      },
    };
    new StageThreeBatchPrebuildContractValidator(this.profile).validate(artifact);
    assert.deepEqual([artifact.activeTopology.counts.modules, artifact.plannedTopology.counts.modules], [42, 44]);
    return { artifact, before, after };
  }

  open() {
    assert(!fs.existsSync(path.join(this.root, PREBUILD)), "batch 012 prebuild is already open");
...
module.exports = { Batch012Prebuild, PREBUILD, PREBUILD_PATHS };
