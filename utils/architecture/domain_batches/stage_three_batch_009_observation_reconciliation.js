...
const { Batch009ObservationManifestTransition, observationProfile, KIND, NEXT } = require("./stage_three_batch_009_observation_transition");

class Batch009ObservationReconciliation {
  build(inputs) {
    ...
    const result = new StageThreeObservationReconciliation({ profile: observationProfile(inputs.prebuild),
      transitionFactory: data => new Batch009ObservationManifestTransition(data) }).build(inputs);
    const record = inputs.cutover.writes.find(item => item.path === "architecture/migration/module_migration_manifest.json");
    ...
    for (const item of inputs.prebuild.preliminaryMetadata.plannedActivationPositions) {
      const old = expected.filter(provider => provider.currentPath === item.sourceProvider && provider.symbol === item.legacySymbol);
    ...
    const removals = approved.compatibility.newActivations.map(item => {
      ...
      return { activationId: activation.id, source: activation.sourceProvider, consumers,
    ...
    return { manifest: result.manifest, facts: immutableRecord({ ...result.facts,
      globalNamespace: { newGlobals: [], removedSymbols: [], mechanismTransitions: result.facts.controlledGlobalTransitions },
      removalDependencies: removals }) };
  }
}

class Batch009ObservationContract extends StageThreeObservationContract {
  constructor(prebuild) { super({ profile: observationProfile(prebuild), kind: KIND, nextGate: NEXT }); }
}

module.exports = { Batch009ObservationReconciliation, Batch009ObservationContract };
