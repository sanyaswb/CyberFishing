...
const { PATHS } = require("./stage_three_batch_010_preflight");
const { serialize } = require("./stage_three_batch_010_planning");

class Batch010CandidateWorkspace {
  constructor() {
    this.parent = fs.realpathSync(os.tmpdir());
    this.root = fs.mkdtempSync(path.join(this.parent, "cyber-batch010-candidate-"));
  ...
  prepare(app, prebuild, projections) {
    ...
    for (const activation of future.activationPositions) {
      this.write(activation.sourceProvider, renderer.render(activation, runtime.transport.symbol));
    }
  ...
  cleanup() {
    ...
    assert(path.basename(resolved).startsWith("cyber-batch010-candidate-"));
    fs.rmSync(resolved, { recursive: true, force: true });
  }
}

module.exports = { Batch010CandidateWorkspace };
