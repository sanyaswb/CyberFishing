...
const { serialize } = require("./stage_three_batch_013_planning");

class Batch013CandidateWorkspace {
  constructor() {
    this.parent = fs.realpathSync(os.tmpdir());
    this.root = fs.mkdtempSync(path.join(this.parent, "cyber-batch013-candidate-"));
  ...
  cleanup() {
    ...
    assert(path.basename(resolved).startsWith("cyber-batch013-candidate-"));
    fs.rmSync(resolved, { recursive: true, force: true });
  }
}

module.exports = { Batch013CandidateWorkspace };
