...
class Batch014CandidateValidation {
  run({ app, report, contract, readOutput, instrumented = false,
    ...
    assert(runtimePosition < earliestNew, "runtime must load before batch 014 providers");
    ...
    for (const target of PROFILE.executionProfile.expectedTargets) {
      ...
      assert.deepEqual(Object.keys(namespace).sort(), target.exports);
      for (const symbol of target.exports) {
        ...
        if (typeof candidate === "function") assert.equal(candidate.name, symbol);
        ...
        behavior.push(...new DomainBehaviorParityHarness().run({
          ...
          cases: BATCH_014_EXECUTABLE_CASES[symbol],
        }));
      }
    }
    assert.equal(behavior.length, plan.scope.modules.reduce((sum, module) =>
      sum + module.exports.reduce((count, symbol) =>
        count + Object.keys(BATCH_014_EXECUTABLE_CASES[symbol]).length, 0), 0));
    ...
    for (const activation of priorContract.activationPositions) {
      ...
      const normalizeBundleHelperNames = text => text.replace(
        /(_[A-Za-z][A-Za-z0-9_]*?)\$\d+/gu, "$1");
      assert.equal(normalizeBundleHelperNames(context[activation.legacySymbol].toString()),
        normalizeBundleHelperNames(before.toString()),
...
module.exports = { Batch014CandidateValidation };
