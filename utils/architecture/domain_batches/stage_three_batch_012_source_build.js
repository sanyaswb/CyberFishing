...
const { Batch012CandidateWorkspace } = require("./stage_three_batch_012_candidate_workspace");
...
const OUTPUT = "architecture/migration/stage_3_batch_012_source_build_validation.json";

class Batch012SourceBuild {
  constructor(root) {
    this.root = path.resolve(root);
    this.app = new Batch012Planning(this.root);
    this.prebuild = new Batch012Prebuild(this.root);
  }
  ...
  projections() {
    return this.app.json(PROFILE.executionProfile.executionPlanPath).scope.modules.map(module =>
      new RepresentationOnlyNamedEsmTarget().project({
        ...
        legacyExposure: PROFILE.reviewedContracts[module.currentPath].legacyExposure,
      }));
  }
  ...
  async candidate({ instrumented = false, failure = null, mutateOutput = output => output,
    ...
    const workspace = new Batch012CandidateWorkspace();
    try {
      ...
      const viteLoader = async () => {
        ...
        return { build: async options => {
          ...
          return vite.build({ ...options, plugins: [...options.plugins, {
            name: "batch012-test-only-evaluation-counter",
            transform(code, id) {
      ...
      const report = await new StageThreeBatchCandidateBuild({
        projectRoot: workspace.root, batchNumber: "012", additionalVirtualModules: [], viteLoader,
        verifyOutput: output => {
          if (failure === "output") throw new Error("injected candidate output rejection");
          validation = new Batch012CandidateValidation().run({ ...mutateOutput(output), app: this.app, instrumented });
  ...
  async prepare() {
    ...
    return {
      ...
      nextGate: "stage-3.12.5-atomic-runtime-cutover",
...
module.exports = { Batch012SourceBuild, OUTPUT };
