...
const OBSERVATION = "architecture/migration/stage_3_batch_013_observation_reconciliation.json";
const MANIFEST = "architecture/migration/module_migration_manifest.json";
const RELEASE = "architecture/migration/stage_3_batch_013_release_transition.json";
  ...
  async run(root, action, { keepPrebuild = false, keepCutover = false, copyTools = false } = {}) {
    ...
    try {
      ...
      if (fs.existsSync(releasePath)) {
        const transition = new (require("./stage_three_batch_013_release_transition").Batch013ReleaseTransition)(temporary);
      ...
      if (fs.existsSync(observationPath)) {
        ...
        const prior = require("./stage_three_batch_013_observation_transition")
          .beforeBatch013Observations(fs.readFileSync(manifest), temporary);
      ...
      if (fs.existsSync(cutoverPath)) {
        ...
        for (const record of cutover.writes) {
          ...
          assert.equal(current && sha(current), record.afterSha256, `batch 013 cutover drift: ${record.path}`);
      ...
      assert(path.basename(resolved).startsWith("cyber-batch013-historical-"));
      fs.rmSync(resolved, { recursive: true, force: true });
    }
  }
}

module.exports = { Batch013HistoricalWorkspace };
