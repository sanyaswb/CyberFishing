...
const { Batch010SourceBuild, OUTPUT: SOURCE_BUILD } = require("./stage_three_batch_010_source_build");
const { Batch010Prebuild, PREBUILD } = require("./stage_three_batch_010_prebuild");
const { PROFILE, PATHS } = require("./stage_three_batch_010_preflight");
const { sha, serialize } = require("./stage_three_batch_010_planning");
...
class Batch010CutoverProjection {
  async prepare(root) {
    const prebuild = new Batch010Prebuild(root).validateOpen();
    ...
    packageContract.stage.current = "3.10";
...
class Batch010AtomicCutover {
    ...
    try {
      for (const write of prepared.writes) {
        ...
        if (!fs.existsSync(parent)) {
          assert.equal(path.relative(this.root, parent).replaceAll("\\", "/"), "src/game/domain/inventory");
          fs.mkdirSync(parent);
          newDirectories.push(parent);
        }
      }
  ...
  async run() {
    assert(!fs.existsSync(path.join(this.root, CUTOVER)), "Batch 010 cutover is already published");
    const prepared = await new Batch010CutoverProjection().prepare(this.root);
    return new Batch010PublicationGate().run(() => {
      new Batch010Prebuild(this.root).validateOpen();
...
module.exports = { Batch010CutoverProjection, Batch010AtomicCutover, Batch010PublicationGate, CUTOVER };
