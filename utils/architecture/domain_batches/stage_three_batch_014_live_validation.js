...
const { Batch014CutoverProjection, CUTOVER } = require("./stage_three_batch_014_cutover");
const { Batch014HistoricalWorkspace } = require("./stage_three_batch_014_historical_workspace");
const { Batch014CandidateValidation } = require("./stage_three_batch_014_candidate_validation");
const { Batch014Planning, sha, serialize } = require("./stage_three_batch_014_planning");
...
const { BATCH_014_EXECUTABLE_CASES } = require("./stage_three_batch_014_behavior_cases");
const { ControlledMetadataTransaction } = require("./controlled_metadata_transaction");

const OUTPUT = "architecture/migration/stage_3_batch_014_live_runtime_validation.json";

class Batch014LiveValidation {
  ...
  verifyPublished() {
    ...
    for (const write of cutover.writes) {
      ...
      const actual = write.path === PATHS.manifest && observationPublished
        ? require("./stage_three_batch_014_observation_transition").beforeBatch014Observations(this.bytes(write.path), this.root)
    ...
    const app = new Batch014Planning(this.root);
    const outputValidation = new Batch014CandidateValidation().run({
    ...
    for (const source of proof.sources) {
      ...
      const approved = new RepresentationOnlyReviewedEsmTarget().project({
        ...
        targetEvaluation: PROFILE.reviewedContracts[source.currentPath].frozenStaticFields
      ...
      for (const symbol of PROFILE.executionProfile.expectedTargets.find(target =>
        ...
        for (const test of Object.values(BATCH_014_EXECUTABLE_CASES[symbol])) test(exported);
      }
    ...
    return {
      ...
      nextGate: "stage-3.14.7-observation-reconciliation",
    };
  }

  async run({ persist = false } = {}) {
    const cutover = this.json(CUTOVER);
    const replay = await new Batch014HistoricalWorkspace().run(this.root,
      root => new Batch014CutoverProjection().prepare(root), { keepPrebuild: true });
...
module.exports = { Batch014LiveValidation, OUTPUT };
