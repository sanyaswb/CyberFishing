...
const { Batch010ObservationManifestTransition, observationProfile, KIND, NEXT } = require("./stage_three_batch_010_observation_transition");

class Batch010ObservationReconciliation {
  build(inputs) {
    ...
    const result = new StageThreeObservationReconciliation({
      ...
      transitionFactory: data => new Batch010ObservationManifestTransition(data),
    }).build(inputs);
...
class Batch010ObservationContract extends StageThreeObservationContract {
  constructor(prebuild) {
    super({ profile: observationProfile(prebuild), kind: KIND, nextGate: NEXT });
  }
}

module.exports = { Batch010ObservationReconciliation, Batch010ObservationContract };
