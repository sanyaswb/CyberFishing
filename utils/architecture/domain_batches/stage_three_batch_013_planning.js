...
const { BATCH_013_MATRIX_DEPENDENCIES } = require("./stage_three_batch_013_focused_test_catalog");
...
class Batch013Planning {
  ...
  matrix() {
    ...
    const matrix = new StageThreeBatchFocusedTestMatrixBuilder(BATCH_013_MATRIX_DEPENDENCIES)
      .build({ executionPlan: plan,
        executionPlanSha256: crypto.createHash("sha256").update(bytes).digest("hex") });
    return new StageThreeBatchFocusedTestMatrixValidator(BATCH_013_MATRIX_DEPENDENCIES)
      .validate(matrix);
  }
}
...
module.exports = { Batch013Planning, PLAN_PATHS, ROLLBACK_PATHS, sha, serialize };
