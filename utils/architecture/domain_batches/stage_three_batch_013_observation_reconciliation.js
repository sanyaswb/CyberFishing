...
const { Batch013ObservationManifestTransition, observationProfile, KIND, NEXT } = require("./stage_three_batch_013_observation_transition");

class Batch013ObservationReconciliation {
  build(inputs) {
    ...
    const result = new StageThreeObservationReconciliation({
      ...
      transitionFactory: data => new Batch013ObservationManifestTransition(data),
    }).build(inputs);
...
class Batch013ObservationContract extends StageThreeObservationContract {
  constructor(prebuild) {
    super({ profile: observationProfile(prebuild), kind: KIND, nextGate: NEXT });
  }
}

module.exports = { Batch013ObservationReconciliation, Batch013ObservationContract };
