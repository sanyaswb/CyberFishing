...
class Batch012CandidateValidation {
  run({ app, report, contract, readOutput, instrumented = false,
    ...
    assert(runtimePosition < earliestNew, "runtime must load before batch 012 providers");
    ...
    for (const target of PROFILE.executionProfile.expectedTargets) {
      ...
      behavior.push(...new DomainBehaviorParityHarness().run({
        ...
        cases: BATCH_012_EXECUTABLE_CASES[symbol],
      }));
    }
    assert.equal(behavior.length, plan.scope.modules.reduce((sum, module) =>
      sum + Object.keys(BATCH_012_EXECUTABLE_CASES[module.exports[0]]).length, 0));
...
module.exports = { Batch012CandidateValidation };
