...
const OBSERVATION = "architecture/migration/stage_3_batch_012_observation_reconciliation.json";
const MANIFEST = "architecture/migration/module_migration_manifest.json";
const RELEASE = "architecture/migration/stage_3_batch_012_release_transition.json";
const sha = bytes => crypto.createHash("sha256").update(bytes).digest("hex");

class Batch012HistoricalWorkspace {
  async run(root, action, { keepPrebuild = false, keepCutover = false, copyTools = false } = {}) {
    ...
    if (fs.existsSync(path.join(projectRoot,
      ...
      return new Batch013HistoricalWorkspace().run(projectRoot,
    ...
    const temporary = fs.mkdtempSync(path.join(parent, "cyber-batch012-historical-"));
    const copy = relative => {
    ...
    try {
      ...
      if (fs.existsSync(releasePath)) {
        const transition = new (require("./stage_three_batch_012_release_transition").Batch012ReleaseTransition)(temporary);
      ...
      if (fs.existsSync(observationPath)) {
        ...
        const prior = require("./stage_three_batch_012_observation_transition")
          .beforeBatch012Observations(fs.readFileSync(manifest), temporary);
      ...
      if (fs.existsSync(cutoverPath)) {
        ...
        for (const record of cutover.writes) {
          ...
          assert.equal(current && sha(current), record.afterSha256, `batch 012 cutover drift: ${record.path}`);
      ...
      assert(path.basename(resolved).startsWith("cyber-batch012-historical-"));
      fs.rmSync(resolved, { recursive: true, force: true });
    }
  }
}

module.exports = { Batch012HistoricalWorkspace };
