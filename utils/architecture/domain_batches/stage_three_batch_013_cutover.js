...
const { Batch013SourceBuild, OUTPUT: SOURCE_BUILD } = require("./stage_three_batch_013_source_build");
...
const { sha, serialize } = require("./stage_three_batch_013_planning");
...
const CUTOVER = "architecture/migration/stage_3_batch_013_runtime_cutover.json";

class Batch013CutoverProjection {
  async prepare(root) {
    const prebuild = new Batch013Prebuild(root).validateOpen();
    const builder = new Batch013SourceBuild(root);
    ...
    const future = { ...runtime, sideEffectReviews: [
      ...runtime.sideEffectReviews, builder.targetSideEffectReview(),
    ], activationPositions: [
    ...
    packageContract.stage.current = "3.13";
...
class Batch013AtomicCutover {
      ...
      for (const write of prepared.writes) {
        ...
        if (!fs.existsSync(parent)) {
          ...
          for (let directory = parent; !fs.existsSync(directory); directory = path.dirname(directory)) {
            ...
            assert(["src/game/domain/fishing/player_pressure",
              "src/game/domain/fishing/stamina"].includes(relative),
  ...
  async run() {
    assert(!fs.existsSync(path.join(this.root, CUTOVER)), "Batch 013 cutover is already published");
    const prepared = await new Batch013CutoverProjection().prepare(this.root);
    return new Batch013PublicationGate().run(() => {
      new Batch013Prebuild(this.root).validateOpen();
...
module.exports = { Batch013CutoverProjection, Batch013AtomicCutover, Batch013PublicationGate, CUTOVER };
