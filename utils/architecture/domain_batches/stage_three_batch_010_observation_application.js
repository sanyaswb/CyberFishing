...
const { Batch010LiveValidation } = require("./stage_three_batch_010_live_validation");
const { Batch010CutoverHistory } = require("./stage_three_batch_010_cutover_history");
const { Batch010ObservationReconciliation, Batch010ObservationContract } = require("./stage_three_batch_010_observation_reconciliation");
const { beforeBatch010Observations, MANIFEST, OUTPUT, PREBUILD, CUTOVER, RUNTIME, KIND, NEXT } = require("./stage_three_batch_010_observation_transition");

const PATHS = Object.freeze({
  prebuild: PREBUILD, cutover: CUTOVER, runtime: RUNTIME,
  live: "architecture/migration/stage_3_batch_010_live_runtime_validation.json",
  audit: "architecture/migration/stage_3_batch_010_audit.json",
...
class Batch010ObservationApplication {
  ...
  async prepare() {
    ...
    new Batch010CutoverHistory(this.root).artifact();
    await new Batch010LiveValidation(this.root).run();
    const data = Object.fromEntries(Object.entries(PATHS).map(([key, file]) => [key, this.json(file)]));
    const pendingBytes = beforeBatch010Observations(this.bytes(MANIFEST), this.root);
    ...
    const result = new Batch010ObservationReconciliation().build(inputs);
    ...
    new Batch010ObservationContract(data.prebuild).validate(artifact, artifact);
  ...
  async check() {
    ...
    new Batch010ObservationContract(result.inputs.prebuild).validate(this.json(OUTPUT), result.artifact);
  ...
  async run() {
    ...
    new ControlledMetadataTransaction({ projectRoot: this.root, failureInjector: this.failureInjector }).commit(writes, () => {
      ...
      assert.deepEqual(beforeBatch010Observations(this.bytes(MANIFEST), this.root), canonicalBytes(result.inputs.before));
...
module.exports = { Batch010ObservationApplication, PATHS };
