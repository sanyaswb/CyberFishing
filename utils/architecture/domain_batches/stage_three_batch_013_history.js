...
const { sha } = require("./stage_three_batch_013_planning");
const { Batch013CutoverHistory } = require("./stage_three_batch_013_cutover_history");
const { PREBUILD } = require("./stage_three_batch_013_prebuild");
const { CUTOVER } = require("./stage_three_batch_013_cutover");
const { MANIFEST, beforeBatch013Observations } = require("./stage_three_batch_013_observation_transition");
const { Batch013ReleaseTransition } = require("./stage_three_batch_013_release_transition");

const STATE = "architecture/migration/stage_3_execution_state.json";

class Batch013History {
  ...
  before(file, provided = this.bytes(file)) {
    let bytes = new (require("./stage_three_batch_014_history").Batch014History)(this.root)
      .before(file, provided);
    if (!this.exists()) return bytes;
    bytes = new Batch013ReleaseTransition(this.root).before(file, bytes);
    if (file === MANIFEST) bytes = beforeBatch013Observations(bytes, this.root);
    if (file === "architecture/guards/known_debt_registry.json") {
      const resolutionPath = "architecture/migration/stage_3_batch_013_known_debt_resolution.json";
    ...
    if (fs.existsSync(path.join(this.root, CUTOVER))) {
      const cutover = new Batch013CutoverHistory(this.root).artifact();
...
module.exports = { Batch013History };
