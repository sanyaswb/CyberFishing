...
const { serialize } = require("./stage_three_batch_012_planning");

class Batch012CandidateWorkspace {
  constructor() {
    this.parent = fs.realpathSync(os.tmpdir());
    this.root = fs.mkdtempSync(path.join(this.parent, "cyber-batch012-candidate-"));
  ...
  cleanup() {
    ...
    assert(path.basename(resolved).startsWith("cyber-batch012-candidate-"));
    fs.rmSync(resolved, { recursive: true, force: true });
  }
}

module.exports = { Batch012CandidateWorkspace };
