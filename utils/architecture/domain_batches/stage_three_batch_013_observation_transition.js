...
const OUTPUT = "architecture/migration/stage_3_batch_013_observation_reconciliation.json";
...
const KIND = "cyber-fishing-stage-3-batch-013-observation-reconciliation";
const NEXT = "stage-3.13.8-full-acceptance-and-browser-smoke";
...
class Batch013ObservationManifestTransition extends StageThreeObservationManifestTransition {
  constructor({ prebuild, cutover, runtime }) {
    ...
    this.removedTargetBuiltinsBySource = {
      "src/core/fishing/fishing_cast_exposure_resolver.js": ["globalThis"],
    };
...
function beforeBatch013Observations(bytes, root) {
  ...
  return new Batch013ObservationManifestTransition({
    prebuild: read(PREBUILD), cutover: read(CUTOVER), runtime: read(RUNTIME),
  }).reverse(Buffer.from(bytes), artifact);
}

module.exports = {
  Batch013ObservationManifestTransition, beforeBatch013Observations,
