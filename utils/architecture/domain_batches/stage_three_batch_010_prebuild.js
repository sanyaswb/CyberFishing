...
const { Batch010Planning, PLAN_PATHS, serialize } = require("./stage_three_batch_010_planning");
...
class Batch010Prebuild {
  constructor(root) {
    this.root = path.resolve(root);
    this.planning = new Batch010Planning(this.root);
    const approved = this.planning.json(PATHS.approvedPlan);
    const plan = this.planning.json(PLAN_PATHS.output);
    const completedPrefix = approved.batches.slice(0, 9).map(batch => batch.id);
    this.profile = new StageThreeBatchPrebuildProfile({
      schemaVersion: 1, contractSchemaVersion: 2, stageLabel: "Stage 3.10.3",
      ...
      unlockCondition: "stage-3.10.4-target-source-and-build-validation",
  ...
  build() {
    ...
    assert.deepEqual(this.bytes(PLAN_PATHS.output), serialize(this.planning.plan()), "execution plan changed before prebuild");
    assert.deepEqual(this.bytes(PROFILE.executionProfile.testMatrixPath), serialize(this.planning.matrix()),
      "focused matrix changed before prebuild");
    ...
    const artifact = {
      ...
      publication: {
        ...
        liveTopologyChanged: false,
        targetSourcesPublished: false,
        rollback: "restore-exact-before-state-and-remove-only-batch-010-prebuild-contract",
      },
    };
    new StageThreeBatchPrebuildContractValidator(this.profile).validate(artifact);
    assert.equal(artifact.activeTopology.counts.modules, 36);
    assert.equal(artifact.plannedTopology.counts.modules, 37);
    return { artifact, before, after };
  }

  open() {
    assert(!fs.existsSync(path.join(this.root, PREBUILD)), "batch 010 prebuild is already open");
    ...
    const protectedPaths = [PATHS.manifest, PATHS.runtimeContract, PATHS.bridgeRegistry,
      PATHS.index, PROFILE.executionProfile.expectedTargets[0].currentPath];
  ...
  validateOpen() {
    ...
    assert.deepEqual(this.bytes(PATHS.executionState), after);
    assert.equal(transition.beforeSha256, artifact.evidence.executionStateBefore.sha256);
    assert.equal(this.json(PATHS.executionState).activeBatchPhase, "prebuild");
    return artifact;
  }
}

module.exports = { Batch010Prebuild, PREBUILD, PREBUILD_PATHS };
