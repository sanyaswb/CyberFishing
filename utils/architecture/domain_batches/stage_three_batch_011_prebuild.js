...
const { Batch011Planning, PLAN_PATHS, serialize } = require("./stage_three_batch_011_planning");
...
class Batch011Prebuild {
  constructor(root) {
    this.root = path.resolve(root);
    this.planning = new Batch011Planning(this.root);
    const approved = this.planning.json(PATHS.approvedPlan);
    const plan = this.planning.json(PLAN_PATHS.output);
    const completedPrefix = approved.batches.slice(0, 10).map(batch => batch.id);
    this.profile = new StageThreeBatchPrebuildProfile({
      schemaVersion: 1, contractSchemaVersion: 2, stageLabel: "Stage 3.11.3",
      ...
      unlockCondition: "stage-3.11.4-target-source-and-build-validation",
  ...
  build() {
    ...
    const artifact = {
      ...
      publication: {
        ...
        rollback: "restore-exact-before-state-and-remove-only-batch-011-prebuild-contract",
      },
    };
    new StageThreeBatchPrebuildContractValidator(this.profile).validate(artifact);
    assert.equal(artifact.activeTopology.counts.modules, 37);
    assert.equal(artifact.plannedTopology.counts.modules, 42);
    return { artifact, before, after };
  }

  open() {
    assert(!fs.existsSync(path.join(this.root, PREBUILD)), "batch 011 prebuild is already open");
...
module.exports = { Batch011Prebuild, PREBUILD, PREBUILD_PATHS };
