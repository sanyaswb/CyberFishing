...
const { serialize } = require("./stage_three_batch_011_planning");

class Batch011CandidateWorkspace {
  constructor() {
    this.parent = fs.realpathSync(os.tmpdir());
    this.root = fs.mkdtempSync(path.join(this.parent, "cyber-batch011-candidate-"));
  ...
  cleanup() {
    ...
    assert(path.basename(resolved).startsWith("cyber-batch011-candidate-"));
    fs.rmSync(resolved, { recursive: true, force: true });
  }
}

module.exports = { Batch011CandidateWorkspace };
