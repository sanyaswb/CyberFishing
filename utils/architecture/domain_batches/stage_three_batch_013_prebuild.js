...
const { Batch013Planning, PLAN_PATHS } = require("./stage_three_batch_013_planning");
...
const PREBUILD = "architecture/migration/stage_3_batch_013_prebuild_contract.json";
...
class Batch013Prebuild {
  constructor(root) {
    this.root = path.resolve(root);
    this.planning = new Batch013Planning(this.root);
    const approved = this.planning.json(PATHS.approvedPlan);
    const plan = this.planning.json(PLAN_PATHS.output);
    const completedPrefix = approved.batches.slice(0, 12).map(batch => batch.id);
    this.profile = new StageThreeBatchPrebuildProfile({
      schemaVersion: 1, contractSchemaVersion: 2, stageLabel: "Stage 3.13.3",
      ...
      unlockCondition: "stage-3.13.4-target-source-and-build-validation",
  ...
  build() {
    ...
    const artifact = {
      ...
      publication: {
        ...
        rollback: "restore-exact-before-state-and-remove-only-batch-013-prebuild-contract",
      },
    };
    new StageThreeBatchPrebuildContractValidator(this.profile).validate(artifact);
    assert.deepEqual([artifact.activeTopology.counts.modules, artifact.plannedTopology.counts.modules], [44, 50]);
    return { artifact, before, after };
  }

  open() {
    assert(!fs.existsSync(path.join(this.root, PREBUILD)), "batch 013 prebuild is already open");
...
module.exports = { Batch013Prebuild, PREBUILD, PREBUILD_PATHS };
