...
const { TRANSITION, RELEASE_PATHS } = require("./domain_batches/stage_three_batch_013_release_transition");
...
const OUTPUT = "architecture/migration/stage_3_batch_013_release_closure.json";
const REGRESSION = "architecture/migration/stage_3_batch_013_release_regression.json";

class Batch013ReleaseFinalizer {
  run(root = path.resolve(__dirname, "../..")) {
    ...
    const verified = new Batch013ReleaseCheck().run(root);
    const accepted = json("architecture/migration/stage_3_batch_013_acceptance_pass.json");
    const browser = json("architecture/migration/stage_3_batch_013_browser_confirmation.json");
    const regression = json(REGRESSION);
    const transition = json(TRANSITION);
    const planPath = "architecture/migration/stage_3_batch_013_execution_plan.json";
    ...
    assert.equal(browser.supplements.sha256,
      sha(read("architecture/migration/stage_3_batch_013_acceptance_pass.json")));
    assert.deepEqual([browser.console.errors, browser.console.warnings], [0, 0]);
    assert.equal(regression.status, "passed");
    assert.equal(regression.releaseVersion, "0.24.50");
    ...
    assert.equal(regression.automatedAcceptance.sha256,
      sha(read("architecture/migration/stage_3_batch_013_automated_acceptance.json")));
    ...
    const closure = {
      ...
      releaseVersion: "0.24.50", previousReleaseVersion: "0.24.49",
      releasePublicationAllowed: true, completedBatchCount: 13,
      completedDomainModuleCount: approved.batches.slice(0, 13)
        .reduce((count, batch) => count + batch.modules.length, 0),
      runtimeTopology: verified.topology,
      acceptance: {
        automatedEvidence: reference("architecture/migration/stage_3_batch_013_automated_acceptance.json"),
        historicalSummary: reference("architecture/migration/stage_3_batch_013_acceptance_pass.json"),
        browserProof: reference("architecture/migration/stage_3_batch_013_browser_confirmation.json"),
      ...
      nextStage: "Stage 3.14.0 — Batch 014 Preflight and Dependency/State Audit",
    };
    assert.equal(closure.completedDomainModuleCount, 41);
    ...
    console.log("Stage 3.13.9 release closure verified: v0.24.50, thirteen completed batches, 41 frozen Domain modules, exact batch-only rollback.");
    return closure;
  }
}

if (require.main === module) {
  try { new Batch013ReleaseFinalizer().run(); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch013ReleaseFinalizer, OUTPUT };
