...
async function run(){const root=path.resolve(__dirname,"../.."),snapshot=new RepositoryContentSnapshot(root),before=snapshot.capture();
  try{const a=await new Batch009LiveValidation(root).run();console.log(`Stage 3.9.5–3.9.6 LIVE PASS: ${a.topology.modules}/${a.topology.activations}/${a.topology.bridges}; exact publication replay, class/state/allocation parity, real script timing; observations pending 3.9.7; release unchanged.`);}
