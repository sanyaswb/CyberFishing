...
const { StageThreeBatch008ObservationReconciliation, StageThreeBatch008ObservationContract } = require("./domain_batches/stage_three_batch_008_observation_reconciliation");
const { Batch008ObservationManifestTransition, MANIFEST, OUTPUT } = require("./domain_batches/stage_three_batch_008_observation_transition");
...
const builder = new StageThreeBatch008ObservationReconciliation();
...
assert(prepared.artifact.guardedConsumers.length > 0, "Existing guarded consumer must retain its classification");
let failures = 0;
...
for (const mutate of [
  ...
  (data) => data.state.releaseVersion = "0.24.45",
...
const contract = new StageThreeBatch008ObservationContract();
...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cyber-fishing-008-observations-"));
try {
  ...
  assert(path.basename(resolved).startsWith("cyber-fishing-008-observations-"));
