...
const { Batch014ReleaseProjection } = require("./project-stage-3-batch-014-release");
...
class Batch014ReleasePublisher {
  run(root = path.resolve(__dirname, "../.."), { failureInjector = null } = {}) {
    const projected = new Batch014ReleaseProjection().run(root);
    ...
    const protectedPaths = [
      ...
      ...require("./domain_batches/stage_three_batch_014_preflight_profile")
        .BATCH_014_PREFLIGHT_PROFILE.executionProfile.expectedTargets.map(item => item.targetPath),
    ];
    ...
    console.log("Stage 3.14.9 release published: v0.24.51, completed 001–014, active null; exact metadata reversal and runtime stability verified.");
  }
}

if (require.main === module) {
  try { new Batch014ReleasePublisher().run(); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch014ReleasePublisher };
