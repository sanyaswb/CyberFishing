...
const { BATCH_014_EXECUTION_PROFILE: PROFILE } =
  require("./domain_batches/stage_three_batch_014_preflight_profile");
const { BATCH_014_MATRIX_DEPENDENCIES: CASES } =
  require("./domain_batches/stage_three_batch_014_focused_test_catalog");

class Batch014MatrixRecorder {
  run(root = path.resolve(__dirname, "../..")) {
    ...
    if (fs.existsSync(target)) assert.deepEqual(fs.readFileSync(target), bytes,
      "Immutable batch 014 matrix drift");
    else new ControlledMetadataTransaction({ projectRoot: root }).commit([
      { relativePath: PROFILE.testMatrixPath, bytes },
    ], () => assert.deepEqual(fs.readFileSync(target), bytes));
    console.log(`Stage 3.14.2 matrix recorded: ${matrix.behaviorCases.length} behavior cases, ` +
...
if (require.main === module) {
  try { new Batch014MatrixRecorder().run(); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch014MatrixRecorder };
