...
class Batch014PlanRecorder {
  run(root = path.resolve(__dirname, "../..")) {
    ...
    assert.equal(plan.compatibility.plannedBridgeRecords.length, 6);
    ...
    if (fs.existsSync(target)) assert.deepEqual(fs.readFileSync(target), bytes,
      "Immutable batch 014 execution plan drift");
    else new ControlledMetadataTransaction({ projectRoot: root }).commit([
      { relativePath: execution.executionPlanPath, bytes },
    ], () => assert.deepEqual(fs.readFileSync(target), bytes));
    console.log("Stage 3.14.1 plan recorded: six targets, six exports/activations, six bridges; live runtime unchanged.");
    return plan;
  }
}

if (require.main === module) {
  try { new Batch014PlanRecorder().run(); }
  catch (error) { console.error(error.stack); process.exitCode = 1; }
}

module.exports = { Batch014PlanRecorder };
