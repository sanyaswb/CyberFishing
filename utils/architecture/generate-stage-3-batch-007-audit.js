...
const PATHS = Object.freeze({
  ...
  output: "architecture/migration/stage_3_batch_007_audit.json",
});
...
function buildArtifact() {
  ...
  const artifact = new StageThreeBatch007DependencyStateAuditBuilder().build({
  ...
  new StageThreeBatch007DependencyStateAuditValidator().validate(artifact);
  return artifact;
}
...
if (require.main === module) {
  ...
  console.log(
    `Stage 3.7.0 audit generated: ${artifact.scope.targetCount} targets, ` +
      `${artifact.closure.existingCumulativeModuleCount}→${artifact.closure.resultingProjectModuleCount} modules, ` +
      `${artifact.performance.hotLoopSensitive.length} hot-loop contracts, ` +
